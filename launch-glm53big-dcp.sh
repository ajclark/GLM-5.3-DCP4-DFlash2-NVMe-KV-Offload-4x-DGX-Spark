...
if [ "$KVTIER" = 1 ]; then
  ...
  case "$KVTIER_MODE" in
    slab) # slab_salt: digest of the bind-mounted overlay code that defines the KV layout/hash (glm-dcp + glm-triton);
          # the slab tier's persisted KV is only valid for the exact overlay it was written under.
          SLAB_SALT=$(cat "$DCP_DIR"/*.py "$KERNELS_DIR"/*.py 2>/dev/null | sha256sum | cut -c1-16)
...
run_docker run -d --name "$NAME" \
  ...
  -e "GLM_DCP_RS_HEADMAJOR=${DCP_RS_HEADMAJOR:-0}" \
  ...
  -e "GLM_SPEC_LOSSY_THINK_IDS=${GLM_SPEC_LOSSY_THINK_IDS:-154841,154842}" \
