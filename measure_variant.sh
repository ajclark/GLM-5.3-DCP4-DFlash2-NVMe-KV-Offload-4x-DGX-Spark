...
say "-- memory"
for h in spark-06c4 spark-365c spark-ddbf spark-a218; do printf "  %s " "$h"; ssh -o BatchMode=yes "${SSH_USER:-$USER}@$h.local" 'awk "/MemAvailable/{printf \"avail=%dMiB\\n\", \$2/1024}" /proc/meminfo'; done | tee -a "$OUT/measure.log"
say "-- log scan (rank 0, 20m)"
ssh -o BatchMode=yes ${SSH_USER:-$USER}@spark-06c4.local "docker logs --since 20m vllm_glm53big 2>&1" | grep -E "Traceback|Error|B12x GLM sparse MLA failed|falling back" | grep -v "NCCL INFO\|error_handling" | sed 's/^(.*pid=[0-9]*) //' | cut -c1-160 | sort | uniq -c | sort -rn | head -6 | tee -a "$OUT/measure.log"
say "== done"
