...
@triton.jit(do_not_specialize=["num_q", "seq_len_kv", "stride_qm", "stride_qh", "stride_qd", "stride_kn", "stride_kd", "stride_wm", "stride_wh", "stride_lm", "stride_ln"])
...
def _describe_launch_failure(kernel_name: str, exc: BaseException, **shapes) -> None:
    """One log line with what matters when a Triton launch fails hours into a process:
    the CUDA error, process uptime, host memory, swap, whether a stream capture is active."""
    import logging, os, time
    info = {"kernel": kernel_name, "error": str(exc)[:160]}
    try:
        with open("/proc/self/stat") as f:
            start_ticks = int(f.read().split()[21])
        with open("/proc/uptime") as f:
            up = float(f.read().split()[0])
        info["uptime_s"] = int(up - start_ticks / os.sysconf("SC_CLK_TCK"))
    except Exception:  # noqa: BLE001
        pass
    try:
        with open("/proc/meminfo") as f:
            mi = {l.split(":")[0]: int(l.split()[1]) for l in f if ":" in l}
        info["MemAvailable_MB"] = mi.get("MemAvailable", 0) // 1024
        info["SwapUsed_MB"] = (mi.get("SwapTotal", 0) - mi.get("SwapFree", 0)) // 1024
        with open("/proc/self/status") as f:
            st = {l.split(":")[0]: l.split()[1] for l in f if l.startswith(("VmRSS", "VmSwap", "Threads"))}
        info.update({k + "_kB" if k.startswith("Vm") else k: v for k, v in st.items()})
    except Exception:  # noqa: BLE001
        pass
    try:
        import torch
        info["stream_capturing"] = bool(torch.cuda.is_current_stream_capturing())
    except Exception:  # noqa: BLE001
        pass
    info.update({k: (tuple(v) if isinstance(v, (list, tuple)) else v) for k, v in shapes.items()})
    logging.getLogger("vllm").error("Triton launch failed: %s", info)
...
def fp8_mqa_logits_triton(
    ...
    try:
        ...
        _describe_launch_failure("_fp8_mqa_logits_kernel", exc, num_q=num_q, seq_len_kv=seq_len_kv, clean_logits=clean_logits)
        raise
    return logits


@triton.jit(do_not_specialize=["num_rows", "logits_width", "stride_qb", "stride_qn", "stride_qh", "stride_qd", "stride_kvb", "stride_kvs", "stride_kvd", "stride_sb", "stride_ss", "stride_wm", "stride_wh", "stride_clb", "stride_cln", "stride_btb", "stride_btk", "stride_lm", "stride_ln"])
...
def fp8_paged_mqa_logits_rowwise_triton(
    ...
    try:
        ...
        _describe_launch_failure("_fp8_paged_mqa_logits_rowwise_kernel", exc, num_rows=num_rows, token_count=token_count)
        raise
    return logits
...
def fp8_paged_mqa_logits_triton(
    ...
    try:
        ...
        _describe_launch_failure("_fp8_paged_mqa_logits_kernel", exc, num_rows=num_rows, token_count=token_count)
        raise
    return logits
