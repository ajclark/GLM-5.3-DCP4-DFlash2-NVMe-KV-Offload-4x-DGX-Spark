#!/usr/bin/env bash
# Wait for spark-ddbf (192.168.1.149) / spark-a218 (192.168.1.31) to answer, then capture the
# previous boot's kernel log, errors, pstore and shutdown reason before anything else runs.
OUT="$(dirname "$0")"; declare -A IP=([spark-ddbf]=192.168.1.149 [spark-a218]=192.168.1.31)
declare -A DONE=()
