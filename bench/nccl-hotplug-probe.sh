...
for h in "${HOSTS[@]}"; do
  st=$(sshq "$h" "docker inspect -f '{{.State.Status}}' vllm_glm53big 2>/dev/null || echo none")
...
for i in 3 2 1 0; do sshq "${HOSTS[$i]}" "cd glm53big && ./run-nccl-hotplug-probe.sh $i $CYCLES" 2>&1 | tail -1 | tee -a "$OUT/driver.log"; sleep 2; done
...
if ! wait_for '"phase": "ready-for-cycle"' 300; then say "probe did not reach ready-for-cycle"; sshq spark-06c4 "docker logs nccl_hotplug_probe 2>&1 | tail -30" | tee -a "$OUT/driver.log"; fi
sshq spark-06c4 "docker logs nccl_hotplug_probe 2>&1" | grep '^{' | tee -a "$OUT/driver.log"
...
grep '^{' "$OUT/spark-06c4.log" | tee "$OUT/rank0.jsonl" | tail -6
