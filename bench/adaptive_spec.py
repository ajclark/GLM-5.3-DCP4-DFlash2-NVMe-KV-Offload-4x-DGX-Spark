...
def main():
    ...
    ap.add_argument("--corpus",type=Path,help="JSON mapping of case names to prompt strings")
    ap.add_argument("--variants",help="Interleaved variants, e.g. fixed7,fixed3,adaptive")
    ...
    args = ap.parse_args()
    prompts = json.loads(args.corpus.read_text()) if args.corpus else PROMPTS
    costs = json.loads(args.costs.read_text()) if args.costs else None
    cases = args.cases.split(',') if args.cases else list(prompts)
    if not all(case in prompts for case in cases):
        ap.error('unknown corpus case')
    caps = [int(k) for k in args.caps.split(',')]
    if not set(caps) <= {1,3,5,7} or not 1 <= args.tokens <= 1024 or not 1 <= args.repeats <= 8:
        ap.error("caps must be 1,3,5,7; tokens 1..1024; repeats 1..8")
    if not 30 <= args.deadline_seconds <= 900:
        ap.error('deadline must be 30..900 seconds')
    variants = [(args.policy,k) for k in caps]
    if args.variants:
        allowed = {**{f'fixed{k}':('fixed',k) for k in (1,3,5,7)},'adaptive':('adaptive',7),'shadow':('shadow',7)}
        try:variants = [allowed[v] for v in args.variants.split(',')]
        except KeyError:ap.error('invalid variant')
        ...
        for repeat in range(args.repeats):
            for case_index,case in enumerate(cases):
                ...
                for mode,cap in order:
                    ...
                    body = request_body(prompts[case],cap,label,args.tokens)
