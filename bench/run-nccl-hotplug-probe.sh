...
# Usage: run-nccl-hotplug-probe.sh <rank 0-3> [cycles]
...
docker run -d --name "$NAME" --restart no \
  ...
  -e NCCL_NET_PLUGIN=hotplug -e NCCL_HOTPLUG_CTL_DIR="$CTL" \
  -e NCCL_IB_DISABLE=0 \
...
echo "$NAME launched rank=$NODE_RANK cycles=$CYCLES plugin=$PLUGIN ctl=$CTL"
