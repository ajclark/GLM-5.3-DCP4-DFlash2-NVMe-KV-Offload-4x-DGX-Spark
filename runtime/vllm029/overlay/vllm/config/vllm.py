...
class VllmConfig:
    ...
    def adjust_dcp_kv_cache_interleave_size(
        ...
        """
        # Worker-local offload does not exchange pages with a P/D peer.
        # Keep the token-striped ownership used by the scheduler and kernels.
        if (self.kv_transfer_config is not None and
                self.kv_transfer_config.kv_connector == "MultiNodeSlabConnector"):
            return
        dcp_size = self.parallel_config.decode_context_parallel_size
        if dcp_size <= 1:
            return
