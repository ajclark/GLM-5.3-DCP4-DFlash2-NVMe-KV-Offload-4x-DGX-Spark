    ...
    if pcp_size * dcp_size > 1:
        ...
        for layer in layers.values():
            ...
            if layer_impl is None:
                continue
            # The process can shard target KV while keeping draft KV replicated.
            # Validate collectives against each implementation's actual ownership.
            layer_dcp_size = getattr(layer_impl, "dcp_world_size", dcp_size)
            if (vllm_config.speculative_config is not None and interleave_size > 1
                    and pcp_size * layer_dcp_size > 1):
