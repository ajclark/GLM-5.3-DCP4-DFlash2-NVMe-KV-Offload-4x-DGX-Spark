...
class BlockTables:
    def __init__(
        ...
        if len(self.cp_sizes) != self.num_kv_cache_groups or any(
            size not in (1, cp_size) for size in self.cp_sizes
        ):
            raise ValueError("KV group CP sizes must use process CP or replicated CP=1")
...
def _gather_block_tables_kernel(
    ...
    num_blocks = tl.minimum(tl.load(group_num_blocks_ptr + req_idx), max_num_blocks)
...
def _compute_slot_mappings_kernel(
    ...
    for i in range(start_idx, end_idx, TRITON_BLOCK_SIZE):
        ...
        if group_cp_size == 1:
            ...
            is_local = tl.full((TRITON_BLOCK_SIZE,), True, tl.int1)
        else:
            # Context parallelism is used.
            virtual_block_size = kv_block_size * group_cp_size
            virtual_block_indices = positions // virtual_block_size
            virtual_block_offsets = positions % virtual_block_size
            is_local = virtual_block_offsets // CP_INTERLEAVE % group_cp_size == cp_rank
        ...
        block_numbers = tl.load(
            ...
            mask=is_local & valid_block & (offset < end_idx),
            other=0,
        ).to(tl.int64)
        ...
        slot_ids = tl.where(mapping_enabled & valid_block, slot_ids, PAD_ID)
