...
class FlashInferMLASparseSM120Impl(MLAAttentionImpl[FlashInferMLASparseMetadata]):
    ...
    # FlashInfer SM120 returns log2(sum(exp(scores))), verified numerically.
    lse_base_on_e = False
    is_sparse = True
    ...
    def __init__(
        ...
        **mla_args,
        ...
        if self.dcp_world_size > 1 and vllm_config.parallel_config.dcp_comm_backend != "ag_rs":
            raise ValueError("Spark ring sparse MLA requires dcp_comm_backend=ag_rs")
        model_type = None
    ...
    def forward_mqa(
        ...
        # FlashInfer divides slot IDs by page size and applies the actual
        # cache stride itself. Keep supported 64/256-token pages, including
        # padded/strided layouts; do not flatten them to one-token pages.
        block_stride_rows = attn_metadata.block_size
        if self.dcp_world_size > 1:
            topk_indices_physical, lengths = triton_filter_and_convert_dcp_index(
                attn_metadata.req_id_per_token[:num_actual_toks],
                attn_metadata.block_table, topk_indices,
            ...
            topk_indices_physical, lengths = triton_convert_req_index_to_global_index(
                attn_metadata.req_id_per_token[:num_actual_toks],
                attn_metadata.block_table, topk_indices,
                BLOCK_SIZE=attn_metadata.block_size,
                BLOCK_STRIDE_ROWS=block_stride_rows,
        ...
        if self.need_to_return_lse_for_decode:
            out, lse = out
            out = out.squeeze(1)
            lse = lse.reshape(num_actual_toks, q.shape[1])
            # Empty local shards contribute neither numerator nor denominator.
            empty = lengths == 0
            out.masked_fill_(empty[:, None, None], 0)
